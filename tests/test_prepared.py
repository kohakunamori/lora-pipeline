...
def test_prepare_requires_captions_by_default(tmp_path) -> None:
...
def test_trigger_only_requires_explicit_opt_in(tmp_path) -> None:
    state = _state(tmp_path)
    result = prepare.run(state, allow_trigger_only=True)
...
def test_prepared_generations_are_content_addressed_and_immutable(tmp_path) -> None:
    state = _state(tmp_path)
    caption = state.project_dir / "raw" / "sample.txt"
    caption.write_text("zz_prepared, red dress\n", encoding="utf-8")
    first = prepare.run(state)
