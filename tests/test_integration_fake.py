...
class FakeTrainer(TrainerBackend):
    def train(self, request: TrainingRequest, *, dry_run: bool = False, verbose: int = 0) -> TrainingResult:
        ...
        (request.run_dir / "config" / "train.toml").write_text("max_train_steps = 3\n", encoding="utf-8")
        ...
        accounting = {
            ...
            "images_seen": request.optimizer_steps,
            "epochs": request.optimizer_steps / 3,
        }
    ...
    def generate(
        ...
        *,
        ...
        for index, case in enumerate(cases):
            ...
            ImageDraw.Draw(image).text((4, 4), f"{case.strength:g}", fill="white")
            image.save(path)
...
def test_complete_style_pipeline_with_fake_backends(tmp_path, monkeypatch) -> None:
    ...
    for folder in ("profiles",):
        shutil.copytree(source_root / folder, test_root / folder)
    ...
    registry = {
        "bases": {
            "fake_base": {
                ...
                "enabled": True,
                "generation_defaults": {"sampler": "euler_a", "scheduler": "normal", "cfg": 4.5, "steps": 2},
            }
        }
    }
    (test_root / "bases" / "registry.yaml").write_text(yaml.safe_dump(registry), encoding="utf-8")
    ...
    raw_before = {path.name: sha256_file(path) for path in (state.project_dir / "raw").glob("*.png")}
    ...
    assert [name for name, _ in results] == ["inspect", "dedup", "caption", "review", "prepare", "preflight", "train", "evaluate"]
    ...
    raw_after = {path.name: sha256_file(path) for path in (state.project_dir / "raw").glob("*.png")}
    ...
    best = yaml.safe_load((run_dir / "best.yaml").read_text(encoding="utf-8"))
    assert best["base"]["sha256"] == sha256_file(base_path)
