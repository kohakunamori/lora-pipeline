...
完成后会显示：候选帧数量、最终保留数量、模糊帧淘汰数量、近重复淘汰数量和曝光异常淘汰数量。

确认创建项目后，只有过滤后的最终帧会被复制到 `projects/<name>/raw/`，临时下载的视频和候选帧会被删除。项目配置会记录原始视频 URL、采样间隔和过滤统计，便于以后追溯数据来源。

视频模式固定创建 **Character** 项目。默认工作流中的 Identity 检查仍然建议保持开启，因为视频中可能出现其他人物、背影、遮挡或远景；Identity/Review 阶段用于在真正训练前继续发现混入的人物和异常帧。

对于人物视频，通常不要把采样间隔设得过小。相邻视频帧的信息增量很低，大量近似姿势会增加过拟合风险。默认每 2 秒采样一次、最多保留 250 帧是偏保守的起点，最终应以过滤后的人物视角和姿势覆盖情况为准。
...
原有的非交互式子命令继续保留，适合自动化或调试。但正常的创建、视频抽帧、训练、恢复、评测、晋升、验证集导入、项目设置和底模管理均可在编号菜单中完成。
